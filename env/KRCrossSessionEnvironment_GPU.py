    ...
    def step(self, step_dict):
        ...
        with torch.no_grad():
            ...
            user_state = self.get_ground_truth_user_state(profile_dict,
                                                          {k: v for k, v in
                                                           self.current_observation['user_history'].items()})
            ...
            behavior_scores, _ = self.immediate_response_model.get_pointwise_scores(user_state,
                                                                                    self.candidate_item_encoding[None,
                                                                                    :, None, :],
            ...
            if done_mask.sum() == len(done_mask):
                ...
                if self.session_count == self.max_n_session:
                    ...
                    self.user_total_return_gap.append(
                        np.sum(np.array(self.user_return_history[-self.max_n_session:]), axis=0))
                    self.env_history['total_return_gap'].append(
                        np.sum(self.env_history['return_day'][-self.max_n_session:]).item())
                    self.session_count = 0
