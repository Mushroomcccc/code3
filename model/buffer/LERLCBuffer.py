...
class LERLCBuffer(BaseBuffer):
    ...
    def reset(self, *reset_args):
        ...
        self.buffer['policy_output']['action_log_prob'] = torch.zeros(self.buffer_size, 1) \
            .to(torch.float).to(self.device)
        self.buffer['policy_output']['llm_action'] = torch.zeros(self.buffer_size, actor.llm_action_dim) \
            .to(torch.long).to(self.device)
        self.buffer['next_step'] = torch.zeros(self.buffer_size, 1).to(torch.long).to(self.device)
    ...
    def sample(self):
        ...
        """
        # get indices
        indices = torch.arange(self.current_buffer_size).to(self.device)
        # observation
        ...
        policy_output = {"state": self.buffer["policy_output"]["state"][indices],
                         ...
                         "llm_action": self.buffer["policy_output"]["llm_action"][indices],
        ...
        next_step = self.buffer['next_step'][indices]

        return observation, policy_output, user_response, done_mask, next_observation, next_step

    def update(self, observation, policy_output, user_feedback, next_observation, next_step):
        """
        @input:
        ...
        self.buffer['policy_output']['llm_action'][indices] = policy_output['llm_action']
        ...
        self.buffer['next_step'][indices] = next_step.long().view(-1, 1)
