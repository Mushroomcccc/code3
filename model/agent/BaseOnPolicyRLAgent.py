...
class BaseOnPolicyRLAgent():
    ...
    def __init__(self, *input_args):
        ...
        self.category_num = np.unique(item_types_np).shape[0]
    ...
    def train(self):
        ...
        for i in tqdm(range(step_offset, step_offset + self.n_iter[-1] // 1)):
            ...
            if i > 0 and i % self.check_episode == 0:
                ...
                episode_report = {k: float(v[0]) if isinstance(v, np.ndarray) else float(v) for k, v in episode_report.items()}
                train_report = {k: float(v[0]) if isinstance(v, np.ndarray) else float(v)  for k, v in train_report.items()}
    ...
    def run_episode_step(self, *episode_args):
        ...
        with torch.no_grad():
            ...
            if torch.sum(done_mask) > 0:
                ...
                # generate reflection
                new_observation_c = deepcopy(new_observation)
                interaction_history = new_observation_c['user_history']['history'][done_mask] 
                single_step_reward = new_observation_c['user_history']['history_is_click'][done_mask]
                total_step = current_step[done_mask]
                feed_dict = {
                    'history': interaction_history,
                    'single_step_reward': single_step_reward,
                    'total_step': total_step,
                }
                self.reflect(feed_dict)
            ...
            if do_buffer_update:
                self.buffer.update(observation, policy_output, user_feedback, update_info['updated_observation'], self.current_step + 1)
