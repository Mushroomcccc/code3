...
def safe_collate(batch):
    # 遍历每个样本，强制将 numpy.int64 转为 Python int
    def convert(x):
        if isinstance(x, np.generic):  # 所有 np.int64、np.float32 等标量
            return x.item()
        elif isinstance(x, np.ndarray):
            # 显式转换 numpy 数组为 torch tensor，指定 dtype
            return torch.tensor(x, dtype=torch.float32 if np.issubdtype(x.dtype, np.floating) else torch.long)
        elif isinstance(x, dict):
            return {k: convert(v) for k, v in x.items()}
        elif isinstance(x, list):
            return [convert(v) for v in x]
        else:
            return x  # int, float, torch.Tensor等保持原样
